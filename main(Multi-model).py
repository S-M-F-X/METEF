...
parser.add_argument('--save_result_every_batch', type=bool, default=False,
...
def main(model_num, seed):
    ...
    solver = Solver(args, config, model_num)
    ...
    print(f'\n>>>>>>>>  Testing : {save_name}  <<<<<<<<\n')
    test_res, test_time, valid_lun, best_lun, best_decay, model_name = solver.test()
    ...
    print(f"\n\nMAE={test_res['mean-MAE']:.6f}, Average training time per epoch: {train_time:.4f} s, "
          f"Effective training epochs: {valid_lun}, Best model at epoch {best_lun}, "
          f"Best model obtained after {best_decay} learning rate decays")
    with open('./Multi-model Comparison Experimental Results.txt', 'a', encoding="utf-8-sig") as f:
        f.write(f"\n{model_name}, seed{seed}, MAE={test_res['mean-MAE']:.6f}, "
                f"Average training time per epoch: {train_time:.4f} s, Effective training epochs: {valid_lun}, "
                f"Best model at epoch {best_lun}, Best model obtained after {best_decay} learning rate decays")
...
if __name__ == '__main__':
    seeds = [1120, 421, 2026, 42, 3407, 1]
    start_model = 1
    end_model = 3

    for model_idx in range(start_model, end_model+1):
        for seed in seeds:
            print(f'==============================Running Model {model_idx}, Seed {seed}==============================')
            main(model_idx, seed)
