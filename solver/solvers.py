...
model_dict = {1: Model, 2: Model, 3: Model}

name_dict = {1: 'Model', 2: 'Model', 3: 'Model'}

class Solver:
    def __init__(self, args, config, model_num):
        self.args = args
        self.config = config
        ...
        self.model_name = name_dict[self.model_num]
        self.model = model_dict[self.model_num](self.config, self.channel).to(self.device)
        if self.args.print:
    ...
    def test(self):
        ...
        if self.args.only_test:
            return test_res, test_time, 0, 0, 0, self.model_name
        else:
            return test_res, test_time, self.valid_lun, self.best_lun, self.best_decay, self.model_name
